...
priority: 50
depends_on: []
description: Re-land the previously reviewer-approved theme-runtime-selector work — expand the theme set from 3 to 6 (3 new palettes + 2 new wall-texture generators) and add a runtime theme override cycled with the T key that re-skins the entire current dungeon in place, with a transient on-screen label. The implementation already exists and was substantively approved on PR #38; it failed only on the (now-fixed) reviewer JSON-envelope bug and never merged.
---

# Theme Runtime Selector — Re-land Expand Themes + In-Place Re-Skin
...
## Re-implementation Provenance & Required Starting Point

This is a **re-land**, not a fresh derivation. An implementation that
satisfies every criterion below already exists and was already produced
and **substantively approved by the independent reviewer**:

- Branch: `task/theme-runtime-selector--20260516T192007b05c56`
- Implementation commit: **`4331741`** ("Task: theme-runtime-selector")
  — an `index.html`-only commit (238 insertions, 11 deletions).
- PR #38. The independent reviewer returned
  `decision: "approved"`, *"All 18 acceptance criteria are met …
  node --check passes … No correctness, security, or spec-conformance
  issues found"*, `findings: []`.

The run was nonetheless marked `failed` because the reviewer prefixed
its JSON verdict with a markdown preamble, so the review-gate's strict
parser died at "line 1, column 2" before reading the (approving)
verdict. That envelope bug was a reviewer-output defect, **not** a code
defect, and has since been fixed generically by the shipped
`harden-review-json-output` task (PR #39, merged to `main` as
`84bfd46`). The theme code itself never landed on `main`.

**Required approach (single decision already made for the
implementer):** start from the existing approved diff rather than
re-deriving it.

1. From a fresh worktree branch off current `main`, apply commit
   `4331741` (e.g. `git cherry-pick 4331741`, or
   `git checkout 4331741 -- index.html`). It is `index.html`-only and
   its base (`a96fc29`, `alt-arrow-strafe`) differs from current `main`
   (`84bfd46`) only by `harden-review-json-output`, which changes only
   `.github/prompts/review.md` — **it does not touch `index.html`** — so
   the theme diff applies with **zero conflicts**. Do **not** pull in
   the branch's stale `.github/prompts/review.md`,
   `specs/tasks/harden-review-json-output.md`, or any other file from
   that branch; the branch predates the `harden-review-json-output`
   merge and its non-`index.html` state is intentionally discarded.
2. Re-verify the result against current `main`: `node --check` on the
   extracted `<script>` body, `git status --porcelain` shows only
   `index.html` modified, and walk the full Acceptance Criteria and
   smoke-test list below confirming each still holds on top of current
   `main`. Fix any drift (there is not expected to be any) minimally and
   note it.

If, on re-verification, any acceptance criterion below is found *not*
met by commit `4331741` (it was approved, so this is not expected),
treat the criteria below — not the old diff — as authoritative and
correct the implementation to satisfy them. The old diff is the
strongly-preferred starting point, but the contract in this spec is what
governs.
...
2. **Six themes are defined.** `THEMES` grows from
   ...
   `themeIdx*3 + materialIdx + 1` wall encoding, the `MM_THEME_COLOR`
   slot, the `forcedThemeIdx` value, and the `T`-cycle order). The three
...
19. **Re-land hygiene (this task only).** The final branch is taken off
    current `main` (`84bfd46` or later) and the only tracked file it
    modifies is `index.html` — `git status --porcelain` and
    `git diff --name-only main` each show exactly `index.html` plus this
    spec file (`specs/tasks/theme-runtime-selector.md`). No stale
    `.github/prompts/review.md`, `specs/tasks/harden-review-json-output.md`,
    or any other file is carried over from the
    `task/theme-runtime-selector--20260516T192007b05c56` branch. The
    `harden-review-json-output` work on `main` is left untouched.
...
- **Reviving the stale `task/theme-runtime-selector--20260516T192007b05c56`
  branch wholesale, re-opening or merging PR #38, or re-touching the
  `harden-review-json-output` change.** Only commit `4331741`'s
  `index.html` content is reused, applied onto a fresh branch off
  current `main` (see Re-implementation Provenance).
...
Line numbers reflect the file as of commit `4331741`'s base; expect
small drift. If applying `4331741` cleanly (the recommended path), these
sites are already wired — this list is for re-verification, not
re-authoring.

- **Generator block** (`index.html:963-1442`): the contract comment at
  963-971 already states "no hardcoded RGB literals" — the two new
  generators must honor it. `genCrystalFacet` and `genHazardStencil`
  sit after the last existing generator (`genCavernCeiling`), before
  the `THEMES` / `THEME_PALETTES` block, so file order stays
  generators-then-tables.
- **`THEMES`** (`index.html:1445`): `'abyss','bunker','sanctum'`
  appended.
- **`THEME_PALETTES`** (`index.html:1452-1474`): entries 3–5 appended;
  0–2 untouched.
- **`TEXTURES`** (`index.html:1480-1496`): the 15 keys from #5 appended
  after the existing 15; existing entries untouched.
- **`WALL_VARIANTS` / `FLOOR_VARIANTS` / `CEIL_VARIANTS`**
  (`index.html:1502-1523`): each extended per #6.
- **`MM_THEME_COLOR`** (`index.html:3376`): extended to 6 entries.
- **Render-mode state** (near `lightingEnabled` /
  `lightingToggledAtMs`, `index.html:1562`): `forcedThemeIdx` and
  `themeToggledAtMs` added.
- **`keydown` handler** (`index.html:410-435`): the `KeyT` case next to
  `KeyL`.
...
- **`castFloorCeiling`** (`index.html:2829-2887`):
  `const forced = forcedThemeIdx;` hoisted to the function top; #11
  applied in both inner `xx` loops.
...
Commit `4331741` already implements both to the reviewer's
satisfaction; re-verify rather than re-author.

## Agent Notes

- **Read first:** `AGENTS.md`, `CLAUDE.md`, this spec (especially the
  **Re-implementation Provenance & Required Starting Point** section),
  and the shipped `specs/tasks/room-themes.md` (the architecture this
  builds on).

- **Order of work (re-land path — strongly preferred):**
  1. Create the assigned worktree branch off current `main`
     (`84bfd46` or later). Confirm `main` HEAD with `git log -1`.
  2. Apply the approved implementation: `git cherry-pick 4331741`
     (or `git checkout 4331741 -- index.html && git add index.html`).
     The commit is `index.html`-only and conflict-free against current
     `main` (its base `a96fc29` differs from `main` only by
     `harden-review-json-output`, which never touches `index.html`).
     Do **not** bring over any other file from that branch.
  3. `git status --porcelain` / `git diff --name-only main` must show
     only `index.html` (plus this spec file) — verify #19.
  4. `node --check` the extracted `<script>` body.
  5. Walk every Acceptance Criterion (1–19) and the smoke-test list
     against current `main` and confirm each holds. Fix only genuine
     drift, minimally, and note any fix in the report.
  6. Browser smoke test (below).
  - If `git cherry-pick 4331741` unexpectedly conflicts, fall back to
    `git checkout 4331741 -- index.html`; if that still does not satisfy
    the criteria, implement to the criteria directly (the contract, not
    the old diff, governs — see Provenance).

- **Common pitfalls (re-verify these in the applied diff):**
  - **Cycle math off-by-one.**
    `forcedThemeIdx = (forcedThemeIdx + 2) % (THEMES.length + 1) - 1;`
    from `-1` → `0`, from `5` → `-1`. Verify all 7 transitions.
  - **Hardcoded literal in a reused generator.** The contract forbids
    them; a grep of `rgba32([0-9]` over the generator block must return
    nothing for the reused generators. If one slipped in, parameterize
    that single value from `pal` and note it; do not fork the generator.
  - **Per-pixel branch in the floor cast keyed off a non-hoisted
    `forcedThemeIdx`.** Confirm `const forced = forcedThemeIdx;` is
    hoisted once per `castFloorCeiling` call and the `forced < 0` path
    is byte-identical to today (bounds check not reordered).
  - **`WALL_VARIANTS` length / index drift.** Length 19, index 0
    `null`, `WALL_VARIANTS[forcedThemeIdx*3 + variant + 1]` in range
    for `forcedThemeIdx ≤ 5`. `|| WALL_VARIANTS[1]` guard retained.
  ...
  - **Label overlap.** The theme label sits at a different `y` than the
    lighting label (`(2,20,..)` rect / `(6,22)` text). Use the row
    below.
  - **New generator wrap seam.** Crystal facets and hazard chevrons
    must tile seamlessly at the 64-px edge. Verify by standing close to
    a long wall.
  - **Touching crypt/foundry/cavern.** Indices 0–2 everywhere must stay
    byte-identical; the theme *distribution* per seed legitimately
    changes (#14). For the legacy-texture regression check, compare a
    forced-`crypt` (`T` to crypt) render against a pre-spec crypt-only
    render, not a natural-mix render.
  - **Stale-branch carry-over.** Do not let `.github/prompts/review.md`,
    `specs/tasks/harden-review-json-output.md`, or any non-`index.html`
    file from the old branch enter the diff (#19).
